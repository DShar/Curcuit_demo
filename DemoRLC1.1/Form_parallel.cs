    ...
    public partial class Form_parallel : Form
    {
        ...
        public void showParameters(int VoltageCurrent)
        {
            ...
            if (VoltageCurrent == 1)
            {
                ...
                this.label_current.Text = circuit.getCurrent().ToString() + "A";
            }
            if (VoltageCurrent == 0)
            {
                this.label_current.Text = "";
                this.label_voltage.Text =  circuit.getVoltageSum().ToString() + "В";
            }
        }
        ...
        private void параллельноеСоединениеToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void последовательноеСоединениеToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Вы уверены, что хотите закрыть текущую схему?",
                "Новая схема", MessageBoxButtons.YesNo, MessageBoxIcon.Question,
                MessageBoxDefaultButton.Button1) == DialogResult.Yes)
        ...
        {
            Parameters param1 = new Parameters(1);
            param1.Owner = this;
            param1.Show();
        }
    }
